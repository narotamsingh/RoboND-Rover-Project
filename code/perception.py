...
def perception_step(Rover):
    ...
    rocks= find_rocks(warped)  # Identify rock
    obstacle = np.absolute(np.float32(threshed) - 1) * mask  # Identify obstacle
    ...
    xpix_nv, ypix_nv = rover_coords(threshed)
    ...
    scale = 2 * dst_size
    xpos = Rover.pos[0] #frame_counter]
    ypos = Rover.pos[1] #ypos[frame_counter]
    yaw = Rover.yaw #[frame_counter]
    
    obstacle_x_world, obstacle_y_world = pix_to_world(xpix_obs, ypix_obs, xpos, ypos, yaw, world_size, scale)
    navigable_x_world, navigable_y_world = pix_to_world(xpix_nv, ypix_nv, xpos, ypos, yaw, world_size, scale)
    ...
    Rover.nav_dists, Rover.nav_angles = to_polar_coords(xpix_nv, ypix_nv)
